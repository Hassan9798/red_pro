...
const SignUpForm = () => {
  ...
  return (
    <div className='shadow-2xl w-full sm:max-w-screen-md min-h-full p-8 sm:p-16 bg-white rounded-3xl sm:rounded-[100px] flex flex-col'>
        <form className='my-auto w-full flex flex-col gap-8 justify-center'>
            <h3 className='text-3xl sm:text-4xl'>Create Your Account</h3>
            <div className='flex flex-col gap-3'>
                <div className='flex flex-col gap-2'>
                    ...
                    />
                    {/* error */}
                    {(formik.touched.fullName && Boolean(formik.errors.fullName)) && typeof formik.errors.fullName === "string" ? (
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    />
                    {/* error */}
                    {(formik.touched.confirmPassword && Boolean(formik.errors.confirmPassword)) && typeof formik.errors.confirmPassword === "string" ? (
                        <div className='text-red-500 text-sm'>{formik.errors.confirmPassword}</div>
                    ) : null}
                </div>
            </div>
            <Button variant={'primary'} size={'xl'} >Create Account</Button>
            <div className='text-base font-medium'>Already Have An Account?&nbsp;<Link href={'/login'} className='text-primary underline decoration-primary'>LOGIN</Link></div>
        </form>
    </div>
  )
}
